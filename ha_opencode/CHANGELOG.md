...
Home Assistant's native LLM platform arrives in **2026.8** — the `llm` integration, the per-domain tool platforms, and the keyed `/api/mcp/<API ID>` endpoints all first ship there. The add-on's native MCP bridge was written against that platform before it existed and had never met it; this release makes the bridge work on both sides of the upgrade.

- **The native Home Assistant MCP bridge now works, on 2026.8 and before it** — the bridge previously targeted `/api/mcp/assist` unconditionally, so on every shipping Home Assistant it hit a 404 and did nothing. It now prefers the keyed `/api/mcp/<API ID>` endpoint and falls back to the configured `/api/mcp` endpoint when the keyed one is not served, logging which of the two situations it found. The fallback is retried periodically rather than latched for the life of the process, so upgrading Home Assistant to 2026.8 under a running add-on is picked up on its own — Core restarts during that upgrade but the add-on does not. If Home Assistant reports that the API ID itself is unknown, that is now surfaced as an error instead of quietly falling back to a different LLM API. Pin either endpoint with `HA_NATIVE_MCP_ENDPOINT_MODE=auto|keyed|configured`.
- **Tool schemas Home Assistant could not serve correctly are repaired in transit** — before 2026.8, tool parameters built from validators such as `cv.string` serialized to an empty `anyOf` member. MCP clients that strictly compile tool schemas cannot parse that, fall back to sending raw arguments, and Home Assistant rejects the call with `extra keys not allowed @ data['__unparsedToolInput']` — which made `GetLiveContext` unusable with any argument ([home-assistant/core#176762](https://github.com/home-assistant/core/issues/176762), fixed upstream by [#176814](https://github.com/home-assistant/core/pull/176814) for 2026.8). The bridge repairs the affected schemas and does nothing on versions that already serve them correctly. `HA_NATIVE_MCP_SANITIZE_SCHEMAS=0` disables the repair.
- **Malformed messages are rejected locally instead of reaching Home Assistant** — every message is validated as JSON-RPC 2.0 before being forwarded, because malformed POSTs to `/api/mcp` have been reported to crash Home Assistant Core ([home-assistant/core#176734](https://github.com/home-assistant/core/issues/176734)). This one is **not** fixed by 2026.8; the upstream fix is still open, so the guard applies on every version.
- **`get_agent_capabilities` says what the running Home Assistant can actually do** — it reports the minimum version for the native LLM platform, whether this instance meets it, and the upstream limitations that still apply to it, so a stalled bridge reads as a known upstream gap rather than a broken configuration. Each limitation is reported against the release that fixes it rather than against one blanket version, so the unfixed crash risk above is still named on 2026.8 while the two issues 2026.8 does fix are not. The AI-component probe was also corrected: `lmstudio` is not a Home Assistant integration and has been dropped, `ovhcloud_ai_endpoints` was added, and conversation agents are now reported separately from AI task providers.
- **Every registered LLM API is reachable from the add-on, not just Assist** — Home Assistant requires admin access for keyed `/api/mcp/<API ID>` endpoints other than Assist, which reads as a wall for an add-on and is not one: the Supervisor calls Home Assistant Core as its own system user, and Home Assistant creates that user in the admin group. A custom LLM API registered by your own integration is therefore testable over `/api/mcp/<your API ID>` with no token and no extra configuration, and an unknown-API-ID error means the ID does not exist rather than that access was refused. Relatedly, `/api/mcp` serves **every** API selected in the Model Context Protocol Server integration — that setting is a multi-select — so leaving the add-on's API ID empty gives you all of them, not one. The bridge also now sends the `MCP-Protocol-Version` header that MCP has required of clients since protocol revision 2025-06-18; Home Assistant does not read it yet, but the Supervisor forwards it.
- **The native LLM development guide matches the shipped upstream code** — `get_ha_llm_development_guide` leads with the exposure-gated `IntentTool` pattern that most of the core `llm.py` platforms use, uses upstream's real import paths, states the 2026.8 requirement, documents the schema pitfall above, and flags the deprecations that make older snippets stale (`async_render_no_api_prompt`, the `helpers/llm.py` refactor in [#176082](https://github.com/home-assistant/core/pull/176082), and `LLMContext.assistant` becoming required).

**To use any of this, two one-time steps are required:** add the **Model Context Protocol Server** integration in Home Assistant (without it, Home Assistant serves no `/api/mcp` routes at all), then turn on **Native Home Assistant MCP bridge** in the add-on options and restart the add-on. Nothing else — the API ID default is correct, no token or environment variable is needed, and version differences are handled automatically from there on. See [Native Home Assistant MCP Bridge](DOCS.md#native-home-assistant-mcp-bridge) for the full procedure and how to verify it.

The bridge remains off by default and still complements OpenCode's own `homeassistant` MCP server rather than replacing it.
