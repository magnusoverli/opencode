...
import { createAuthorization, equal, SCOPE, HttpError, requireThat } from "./ha-facing-auth.js";
 ...
 * Core HTTP network carries confidential-client credentials and OAuth tokens;
 * it must not be host-published. Browser setup/consent is HTTPS HA Ingress only.
...
export async function startHaFacing({ state, resourceUrl, ingressSecret, verifyAdmin, backendFactory,
  ...
  const auth = createAuthorization(state, resource.href);
  ...
  async function reap() {
    ...
    await Promise.all([...sessions.values()].filter((session) =>
      ...
      !state.data.grants.some((grant) => grant.id === session.grantId && grant.expires > now)
    ).map(dispose));
  }
  function handler(fn) {
    return (req, res) => {
  ...
  const core = createServer({ requestTimeout: 20000, headersTimeout: 10000, maxHeaderSize: 16384 }, handler(async (req, res, url) => {
    ...
    const grant = bearer && auth.verify(bearer[1]);
    if (!grant) {
    ...
    if (!session) {
      ...
      try {
        ...
        requireThat(!closing && auth.verify(bearer[1]), 401, "invalid_token");
  ...
  const ipc = createServer({ requestTimeout: 20000, headersTimeout: 10000, maxHeaderSize: 16384 }, handler(async (req, res, url) => {
    ...
    if (req.method === "GET" && url.pathname === "/ha-mcp/status") {
      return json(res, 200, { ready: Boolean(state.data.client), url: resource.href });
    }
    ...
    try {
      ...
      requireThat(external.origin === origin && external.protocol === "https:" && !external.username && !external.password, 403, "invalid_ingress_origin");
